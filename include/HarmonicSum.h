...
public:
  ...
  std::complex<long double> HS(int i, int j, int k,
...
private:
  ...
  long double Zeta(int i) { return gsl_sf_zeta_int(i); }

  std::complex<long double> B(int i, std::complex<long double> z) {
    return (1. / std::pow(2., (long double)i + 1.) *
            (PolyGamma(i, (1. + z) / 2.) - PolyGamma(i, z / 2.)));
  }
  // Important Costants
  long double zeta2;
  long double zeta3;
  ...
  std::complex<long double> PolyGamma(int i, std::complex<long double> z) {
    if (i == 0) {
      ...
      return (SUB + std::log(ZZ) - 0.5 * RZ -
              DZ / 5040. * (420. + DZ * (-42. + DZ * (20. - 21. * DZ))));
    } else {
      int K1, K2;
      ...
      return (
          SUB +
          std::pow(-1., i + 1) * std::pow(RZ, i) *
              (A1 +
               RZ * (A2 +
                     RZ * (A3 + DZ * (A4 + DZ * (A5 + DZ * (A6 + A7 * DZ)))))));
    }
  }
};

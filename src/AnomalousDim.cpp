...
void AnomDimensions::ComputeGamma(std::complex<double> N, int order)
{
    if(order>=0)
    {
...
void AnomDimensions::sums(std::complex<double> N)
{
    g1s.S1 = HAP.HS(1,N);
    g1s.S2 = HAP.HS(2,N);
    g1s.S3 = HAP.HS(3,N);
...
std::complex<double> AnomDimensions::gammagg0(std::complex<double> N)
{
    return (-1./(4.*M_PI)*(CA*(4.*(HAP.HS(1,N-2.)-2.*HAP.HS(1,N-1.)-
       2.*HAP.HS(1,N+1.)+HAP.HS(1,N+2.)+3.*HAP.HS(1,N))-11./3.)+2./3.*NF));
}

std::complex<double> AnomDimensions::gammagS0(std::complex<double> N)
{
    return -1./(4.*M_PI)*(2.*CF*(2.*HAP.HS(1,N-2.)-4.*HAP.HS(1,N-1.)-
                HAP.HS(1,N+1.)+3.*HAP.HS(1,N)));
}

std::complex<double> AnomDimensions::gammaSg0(std::complex<double> N)
{
    return -1./(4.*M_PI)*(2.*NF*(HAP.HS(1,N-1.)+4.*HAP.HS(1,N+1.)-
                2.*HAP.HS(1,N+2)-3.*HAP.HS(1,N)));
}

std::complex<double> AnomDimensions::gammansplus0(std::complex<double> N)
{
    return -1./(4.*M_PI)*CF*(2.*(HAP.HS(1,N-1.)+HAP.HS(1,N+1.))-3.);
}

// NLO important definition
void AnomDimensions::DEF1(std::complex<double> N)
{
...
std::complex<double> AnomDimensions::gammagg1(std::complex<double> N)
{
    return (CA*CA*PGGA+0.5*NF*(CA*PGGB+CF*PGGC))*4./pow(4*M_PI,2);
}

std::complex<double> AnomDimensions::gammagS1(std::complex<double> N)
{
    return (CF*CF*PGQA+CF*CA*PGQB+0.5*NF*CF*PGQC)*4./pow(4*M_PI,2);
}

std::complex<double> AnomDimensions::gammaSg1(std::complex<double> N)
{
    return 0.5*NF*(CA*PQGA+CF*PQGB)*4./pow(4*M_PI,2);
}

std::complex<double> AnomDimensions::gammaps1(std::complex<double> N)
{
    return  (0.5*NF*CF*PPSA*4./pow(4*M_PI,2));
}

std::complex<double> AnomDimensions::gammansplus1(std::complex<double> N)
{
    return CF *((CF-CA/2.)*PNPA+CA*PNSB+0.5*(NF)*PNSC)/pow(4*M_PI,2);
}

std::complex<double> AnomDimensions::gammansminus1(std::complex<double> N)
{
    return CF*((CF-CA/2.)*PNMA+CA*PNSB+0.5*NF*PNSC)/(pow(4*M_PI,2));
}

// NNLO important definitions
void AnomDimensions::DEF2(std::complex<double> N)
{
    std::complex<double> B1M;
    std::complex<double> A0 = -g1s.S1M;
    std::complex<double> B1 = -g1s.S1 * g1s.NI;
    ...
    std::complex<double> B11 = - g1s.S11*g1s.N1I;
    std::complex<double> B12 = - g1s.S12*g1s.N2I;
    std::complex<double> B2  = (pow(g1s.S1,2)+g1s.S2)*g1s.NI;
    std::complex<double> B2M = (pow(g1s.S1M,2)+g1s.S2M)*g1s.NMI;
    std::complex<double> B21 = (pow(g1s.S11,2)+g1s.S21)*g1s.N1I;
    std::complex<double> B3  = -(pow(g1s.S1,3)+ 3.*g1s.S1*g1s.S2+2.*g1s.S3)*g1s.NI;
    std::complex<double> B31 = -(pow(g1s.S11,3)+3.*g1s.S11*g1s.S21+2.*g1s.S31)*g1s.N1I;
    std::complex<double> B4  = (pow(g1s.S1,4)+6.*pow(g1s.S1,2)*g1s.S2+8.*g1s.S1*g1s.S3+
            3.*pow(g1s.S2,2)+6.*g1s.S4)*g1s.NI;

    std::complex<double> C0  = g1s.NI;
    std::complex<double> CM  = g1s.NMI;
    std::complex<double> C1  = g1s.N1I;
    std::complex<double> C2  = g1s.N2I;
    std::complex<double> C3  = 1./(N+3.);
    std::complex<double> C4  = 1./(N+4.);
    std::complex<double> D1  = -g1s.NI2;
    std::complex<double> D1M = -g1s.NMI2;
    std::complex<double> D11 = -g1s.N1I2;
    std::complex<double> D2  = 2.*g1s.NI3;
    std::complex<double> D21 = 2.*g1s.N1I3;
    std::complex<double> D3  = -6.*g1s.NI2*g1s.NI2;
    std::complex<double> D31 = -6.*g1s.N1I2*g1s.N1I2;
    std::complex<double> D4  = 24.*g1s.NI2*g1s.NI3;
    std::complex<double> D41 = 24.*g1s.N1I2*g1s.N1I3;
    std::complex<double> E1  = g1s.S1*g1s.NI2+(g1s.S2-gsl_sf_zeta(2))*g1s.NI;
    std::complex<double> E11 = g1s.S11*g1s.N1I2+(g1s.S21-gsl_sf_zeta(2))*g1s.N1I;
    std::complex<double> E2  = 2.*(-g1s.S1*g1s.NI3+(gsl_sf_zeta(2)-g1s.S2)*g1s.NI2
            -(g1s.S3-gsl_sf_zeta(3))*g1s.NI );

    std::complex<double> PP2 =+ 1174.898*A0+1295.384+714.1*B1-522.1*C3+243.6*C2-3135.
    ...
    std::complex<double> PM2 = +1174.898*A0+1295.470+714.1*B1-433.2*C3+297.0*C2-3505.
     ...
     std::complex<double> PSS2 = -163.9*(B1M-B1)-7.208*(B11-B12)+4.82*(C3-C4)- 43.12*
      (C2-C3)+44.51*(C1-C2)+151.49*(C0-C1)+178.04*D1+6.892*D2-40./27.*(2.*D3-D4)-173.1*E1+
      46.18 * E2;

     std::complex<double> PF2 = -( 17./72.-2./27.*g1s.S1-10./27.*g1s.S2+2./9.*g1s.S3-
     ...
     std::complex<double> PS1 = -3584./27.* (D1M-D1)-506.* (CM-C0)+ 160./27.*(D4-D41)-
    ...
    std::complex<double> PS2 = 256./81.*(CM-C0)+32./27.*(D3-D31)+17.89*(D2-D21)+61.75*
      (D1-D11)+1.778*(B2-B21)+5.944*(B1-B11)+100.1*(C0-C1)-125.2*(C1-C2)+49.26*(C2-C3)-12.59
      *(C3-C4)-1.889*(E1-E11);

    std::complex<double> QG1 = -896./3. *D1M-1268.3*CM+536./27.*D4-44./3.*D3+881.5*D2+
      424.9*D1+100./27.*B4-70./9.*B3-120.5*B2+104.42*B1+2522.*C0-3316.*C1+2126.*C2+1823.*E1
      -25.22*E2-252.5*D31;

    std::complex<double> QG2 = 1112./243.*CM -16./9.*D4-376./27.*D3 -90.8*D2-254.0*D1+
      20./27.*B3+200./27.*B2-5.496*B1-252.0*C0+158.0*C1+145.4*C2-139.28*C3-53.09*E1-80.616*
      E2-98.07*D21+11.70*D31;

    std::complex<double> GQ0 = 1189.3*D1M+6163.1*CM-4288./81.*D4+1568./9.*D3-1794.*D2+
      4033.*D1+400./81.*B4+2200./27.*B3+606.3*B2+2193.*B1-4307.*C0+489.3*C1+1452.*C2+146.*C3
      -447.3*E2-972.9*D21;

    std::complex<double> GQ1 = 71.082*D1M-46.41*CM+128./27.*D4+704/81.*D3+20.39*D2+174.8
      *D1-400./81.*B3-68.069*B2-296.7*B1-183.8*C0+33.35*C1-277.9*C2+108.6*D21-49.68*E1;

    std::complex<double> GQ2 = (64.*(-CM+C0+2.*C1)+320.*(B1M-B1+0.8*B11)+96.*(B2M-B2+0.5
                *B21))/27.;

    std::complex<double> GG0 = 2675.8*D1M+14214.*CM-144.*D4+72.*D3-7471.*D2+274.4*D1-20852.
      *C0+3968.*C1-3363.*C2+4848.*C3+7305.*E1+8757.*E2+3589.*B1+4425.894+2643.521*A0;

    std::complex<double> GG1 = 157.27*D1M+182.96*CM+512./27.*D4+832./9.*D3+491.3*D2+1541.
      *D1-350.2*C0+755.7*C1-713.8*C2+559.3*C3+26.15*E1-808.7*E2-320.*B1-528.723-412.172*A0;

    std::complex<double> GG2 = -680./243.*CM-32./27.*D3+9.680*D2-3.422*D1-13.878*C0+153.4
...
std::complex<double> AnomDimensions::gammagg2(std::complex<double> N)
{
    return P2GGN/pow(4*M_PI,3);
}

std::complex<double> AnomDimensions::gammagS2(std::complex<double> N)
{
    return P2GQN/pow(4*M_PI,3);
}

std::complex<double> AnomDimensions::gammaSg2(std::complex<double> N)
{
    return P2QGN/pow(4*M_PI,3);
}

std::complex<double> AnomDimensions::gammaps2(std::complex<double> N)
{
    return P2PSN/pow(4*M_PI,3);
}

std::complex<double> AnomDimensions::gammansplus2(std::complex<double> N)
{
    return P2PLSN/pow(4*M_PI,3);
}

std::complex<double> AnomDimensions::gammansminus2(std::complex<double> N)
{
    return P2MINN/pow(4*M_PI,3);
}

std::complex<double> AnomDimensions::gammansval2(std::complex<double> N)
{
    return P2VALN/pow(4*M_PI,3);
}

...
// Inner Product
std::complex<long double> operator * (const std::vector<std::complex<long double>> &c1,
...
std::complex<long double> expm1 (const std::complex<long double> &z)
{
    ...
    return std::complex<long double> (expm1_x - 2.0*exp_x*sin_y_over_two*
...
std::complex<long double> log1p (const std::complex<long double> &z)
{
    ...
    return std::complex<long double> (std::log1p(x)+0.5*std::log1p(y_over_xp1*y_over_xp1),
            std::atan2 (y,xp1));
}
...
std::complex<long double> LogGamma (std::complex<long double> z)
{
    ...
    if (x >= 0.5)
    {
      ...
      const long double c[15] = {0.99999999999999709182, 57.156235665862923517,
      ...
      const std::complex<long double> log_Gamma_z = log_sqrt_2Pi+std::log(sum)+z_m_0p5*
    ...
    else if (y >= 0.0)
    {
      ...
      const std::complex<long double> log_const(-M_LN2l,M_PI_2l),i_Pi(0.0,M_PIl);
      const std::complex<long double> eps = z-n;
      const std::complex<long double> log_sin_Pi_z = (y>110)?(-i_Pi*z+log_const):(log(sin(M_PIl*eps))-i_Pi*n);
      const std::complex<long double> log_Gamma_z = log_Pi-log_sin_Pi_z-LogGamma(1.0 - z);
...
std::complex<long double> Gamma_inv (const std::complex<long double> &z)
{
    ...
    if (x >= 0.5)
    {
      ...
      const std::complex<long double> Gamma_inv_z = exp (z_pg_m0p5-z_m_0p5*log(z_pg_m0p5)
              -log_sqrt_2Pi)/sum;
...
std::complex<long double> LBesselJ(long double k, std::complex<long double> z)
{
  ...
  const long double risreal=static_cast<long double>(std::real(ris)),risimag=static_cast<long double>(std::imag(ris));
        ...
        bool warn)
{
    ...
    else
    {
      for (i=0;i<num;i++)
      {
        long double il=static_cast<long double> (i);
        long double factl=static_cast<long double> (gsl_sf_fact(i));
        error = std::sqrt(M_PIl/(2.*z))*std::exp(-z)*std::exp(LogGamma(0.5+il+nu)+LogGamma(0.5+il-nu)
...
std::complex<long double> Gamma_ratio_diff_small_eps(const std::complex<long double> &z,
        const std::complex<long double> &eps)
{
    ...
    else
    {
      if (eps!=0.0)
      {
        const std::complex<long double> Pi_eps = M_PIl*eps,term = std::sin (Pi_eps)/std::tan (M_PIl*(z-n));
        const std::complex<long double> T1_eps_z = (std::cos(Pi_eps)+term)*Gamma_ratio_diff_small_eps(1.0-z,-eps);
        const std::complex<long double> sin_Pi_2_eps = std::sin(M_PI_2l*eps);
        const std::complex<long double> T2_eps_z = (2.0*sin_Pi_2_eps*sin_Pi_2_eps-term)/eps;
        const std::complex<long double> T_eps_z = T1_eps_z+T2_eps_z;

        return (T_eps_z/(1.0 - eps*T_eps_z));
      }
      else
        return (Gamma_ratio_diff_small_eps(1.0-z,0.0)-M_PIl/tan(M_PIl*(z-n)));
    }
}

std::complex<long double> Gamma_inv_diff_eps(const std::complex<long double> &z,
        const std::complex<long double> &eps)
{
    const std::complex<long double> eps_pz = z + eps;
    ...
    else if (eps_pz!=z)
    {
      ...
      else
      {
        ...
        const long double eps_pz_neg_int_distance = inf_norm(eps_pz+std::abs (m));
...
std::complex<long double> A_sum_init(const int m,const std::complex<long double> &eps,
...
std::complex<long double> log_A_sum_init(const int m,const std::complex<long double> &eps)
{
    ...
    else
    {
      ...
      for (int n=2 ; n<m ; n++) log_fact += std::log(static_cast<long double>(n));

      return (m%2==0)?(log_fact):(log_fact+i_Pi);
    }
}

std::complex<long double> B_sum_init_PS_one(const std::complex<long double> &a,const std::complex<long double> &b,
        const std::complex<long double> &c,const std::complex<long double> &Gamma_c,
        const std::complex<long double> &Gamma_inv_one_meps,const std::complex<long double> &Gamma_inv_eps_pa_pm,
        const std::complex<long double> &Gamma_inv_eps_pb_pm,const std::complex<long double> &one_minus_z,
        const int m,const std::complex<long double> &eps)
{
    ...
    if (inf_norm_eps>0.1)
    {
      const std::complex<long double> Gamma_inv_eps_pm_p1 = phase*std::sin(Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm);
      std::complex<long double> prod1 = Gamma_inv_one_meps*Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm;
      for (int n=0 ; n<m ; n++) prod1 *= (a+n)*(b+n)/(n+1.0);
      const std::complex<long double> prod2 = Gamma_inv(a)*Gamma_inv(b)*Gamma_inv_eps_pm_p1*std::pow(one_minus_z,eps);
    ...
    else
    {
      ...
      const std::complex<long double> Gamma_inv_eps_pm_p1 = (is_eps_non_zero)?(phase*std::sin(Pi_eps)/
              (Pi_eps_pm*Gamma_inv_one_meps_mm)):(Gamma_inv_mp1);
      const std::complex<long double> Gamma_inv_a_pm = Gamma_inv(a_pm),Gamma_inv_b_pm = Gamma_inv(b_pm);
      const std::complex<long double> z_term = (is_eps_non_zero)?(expm1(eps*log (one_minus_z))/eps):(log(one_minus_z));

      const std::complex<long double> prod1 = Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm*(Gamma_inv_mp1*Gamma_inv_diff_eps(1.0,-eps)+
                                              Gamma_inv_diff_eps(m+1,eps));
      const std::complex<long double> prod2 = Gamma_inv_eps_pm_p1*(Gamma_inv_eps_pb_pm*Gamma_inv_diff_eps(a_pm,eps)+Gamma_inv_a_pm
                                             *Gamma_inv_diff_eps(b_pm,eps));
      const std::complex<long double> prod3 = Gamma_inv_a_pm*Gamma_inv_b_pm*Gamma_inv_eps_pm_p1*z_term;

      const std::complex<long double> res = Gamma_c*prod_ab*(prod1-prod2-prod3);
      if (isfinite (res))
        return res;
      else
      {
        const std::complex<long double> Gamma_inv_eps_pm_p1 = phase*sin(Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm);
        std::complex<long double> prod1 = Gamma_inv_one_meps*Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm;
        for (int n=0 ; n<m ; n++) prod1 *= (a+n)*(b+n)/(n+1.0);
        const std::complex<long double> prod2 = Gamma_inv(a)*Gamma_inv(b)*Gamma_inv_eps_pm_p1*pow(one_minus_z,eps);
        const std::complex<long double> res_default = Gamma_c*(prod1 - prod2)/eps;
        return res_default;
      }
    }
}

std::complex<long double> B_sum_init_PS_infinity (const std::complex<long double> &a,const std::complex<long double> &c,
			   const std::complex<long double> &Gamma_c,const std::complex<long double> &Gamma_inv_cma,
			   const std::complex<long double> &Gamma_inv_one_meps,const std::complex<long double> &Gamma_inv_eps_pa_pm,
			   const std::complex<long double> &z,const int m,const std::complex<long double> &eps)
{
    ...
    {
      const std::complex<long double> Gamma_inv_eps_pm_p1 = phase*std::sin(Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm);
      std::complex<long double> prod1 = Gamma_inv_cma*Gamma_inv_eps_pa_pm*Gamma_inv_one_meps;
      std::complex<long double> prod2 = Gamma_inv(a)*Gamma_inv_cma_meps*Gamma_inv_eps_pm_p1*std::pow(-z,-eps);
    ...
    else
    {
      ...
      std::complex<long double> prod_eps_pa_mc_p1_n0 = (is_n0_here)?(1.0):(0.0),prod_eps_pa_mc_p1 = 1.0;
      std::complex<long double> sum = 0.0;
      for (int n=0 ; n<m ; n++)
      {
        const std::complex<long double> a_pn=a+n, a_mc_p1_pn=a_mc_p1+n, eps_pa_mc_p1_pn=eps_pa_mc_p1+n;
        prod_a *= a_pn;
      ...
      const std::complex<long double> Gamma_inv_eps_pm_p1 = (is_eps_non_zero)?(phase*sin(Pi_eps)/
              (Pi_eps_pm*Gamma_inv_one_meps_mm)):(Gamma_inv_mp1);
      const std::complex<long double> sum_term = (is_eps_non_zero)?(expm1(sum)/eps):(sum);
      const std::complex<long double> prod_diff_eps = prod_eps_pa_mc_p1_n0+prod_a_mc_p1*sum_term;

      const std::complex<long double> z_term = (is_eps_non_zero)?(expm1(-eps*std::log (-z))/eps):(-std::log (-z));
      const std::complex<long double> Gamma_inv_a_pm = Gamma_inv(a_pm),Gamma_prod1 = Gamma_inv_cma*Gamma_inv_eps_pa_pm;
      const std::complex<long double> prod1 = Gamma_prod1*Gamma_inv_mp1*(Gamma_inv_diff_eps(1.0,-eps)*prod_eps_pa_mc_p1
              -Gamma_inv_one_meps*prod_diff_eps);
      const std::complex<long double> prod_2a = Gamma_prod1*Gamma_inv_diff_eps(m+1,eps);
      const std::complex<long double> prod_2b = Gamma_inv_cma*Gamma_inv_eps_pm_p1*Gamma_inv_diff_eps(a_pm,eps);
      const std::complex<long double> prod_2c = Gamma_inv_eps_pm_p1*Gamma_inv_a_pm*(Gamma_inv_diff_eps(cma,-eps)+
              Gamma_inv_cma_meps*z_term);
      const std::complex<long double> prod2 = prod_eps_pa_mc_p1*(prod_2a-prod_2b-prod_2c);
      ...
      else
      {
        const std::complex<long double> Gamma_inv_eps_pm_p1 = phase*std::sin(Pi_eps)/(Pi_eps_pm*Gamma_inv_one_meps_mm);
        std::complex<long double> prod1 = Gamma_inv_cma*Gamma_inv_eps_pa_pm*Gamma_inv_one_meps;
        std::complex<long double> prod2 = Gamma_inv(a)*Gamma_inv_cma_meps*Gamma_inv_eps_pm_p1*pow(-z,-eps);
...
void cv_poly_der_tab_calc(const std::complex<long double> &a,const std::complex<long double> &b,
...
long double cv_poly_der_calc(const long double cv_poly_der_tab[],const long double x)
{
    const long double Px = cv_poly_der_tab[0]+x*(cv_poly_der_tab[1]+x*
...
std::complex<long double> hyp_PS_zero(const std::complex<long double> &a,const std::complex<long double> &b,
        const std::complex<long double> &c,const std::complex<long double> &z)
{
...
std::complex<long double> hyp_PS_one(const std::complex<long double> &a,const std::complex<long double> &b,
        const std::complex<long double> &c,const std::complex<long double> &one_minus_z)
{
    const int m = static_cast<int> (std::rint(std::real(c-a-b)));
    const int phase = (m%2==0)?(1):(-1);
    const int m_m1  = m-1, m_p1 = m+1;

    const std::complex<long double> eps = c-a-b-m, eps_pm = eps+m, eps_pm_p1 = eps_pm+1.0;
    const std::complex<long double> a_pm = a+m, b_pm = b+m, one_meps = 1.0-eps, one_meps_mm = one_meps-m;
    const std::complex<long double> eps_pa = a+eps, eps_pb = b+eps, eps_pa_pm = eps_pa+m;
    const std::complex<long double> eps_pb_pm = eps_pb+m, Pi_eps = M_PI*eps, Gamma_c = 1.0/Gamma_inv(c);
    ...
    const std::complex<long double> Gamma_prod = Gamma_c*Gamma_inv_eps_pa_pm*Gamma_inv_eps_pb_pm;
    const std::complex<long double> Gamma_inv_one_meps = Gamma_inv (one_meps);

    const std::complex<long double> A_first_term = (m > 0) ? (Gamma_prod*A_sum_init (m,eps,Gamma_inv_one_meps)) : (0.0);
    ...
    const std::complex<long double> B_first_term = B_sum_init_PS_one(a,b,c,Gamma_c,Gamma_inv_one_meps,
    ...
    std::complex<long double> B_extra_term = prod_B*Gamma_prod*Gamma_inv_one_meps;
    ...
    while (possible_false_cv||(inf_norm(B_term)>B_prec))
    {
      ...
      const std::complex<long double> eps_pm_p1_pn = eps_pm_p1+n, n_p1_meps = one_meps+n;
      const std::complex<long double> eps_pa_pm_pn = eps_pa_pm+n, eps_pb_pm_pn = eps_pb_pm+n;
...
std::complex<long double> hyp_PS_infinity (const std::complex<long double> &a,const std::complex<long double> &b,
        const std::complex<long double> &c,const std::complex<long double> &z)
{
  const int m = static_cast<int> (std::rint(real(b-a)));
  const int phase = (m%2==0)?(1):(-1), m_m1 = m-1, m_p1 = m+1;
  const std::complex<long double> eps = b-a-m, a_mc_p1 = 1.0-c+a, one_meps = 1.0-eps, one_meps_mm = one_meps-m;
  const std::complex<long double> a_pm = a+m,a_mc_p1_pm = a_mc_p1+m,cma = c-a;
  const std::complex<long double> eps_pa = eps+a, eps_pm_p1 = eps+m+1;
  const std::complex<long double> eps_pa_mc_p1_pm = eps+a_mc_p1_pm, Pi_eps = M_PIl*eps;
  const std::complex<long double> eps_pa_pm = eps_pa+m;
  const std::complex<long double> Gamma_c = 1.0/Gamma_inv(c), Gamma_inv_eps_pa_pm = Gamma_inv(eps_pa_pm);
  const std::complex<long double> Gamma_inv_cma = Gamma_inv(cma), z_inv = 1.0/z;
  const std::complex<long double> pow_mz_ma = std::pow(-z,-a), Gamma_inv_one_meps = Gamma_inv(one_meps);
  const std::complex<long double> Gamma_prod = Gamma_c*Gamma_inv_cma*Gamma_inv_eps_pa_pm;
  ...
  const std::complex<long double> B_first_term = B_sum_init_PS_infinity(a,c,Gamma_c,Gamma_inv_cma,
  ...
  {
    const int n_pm_p1 = n+m_p1, n_p1 = n+1;
    const std::complex<long double> a_pm_pn = a_pm+n, a_mc_p1_pm_pn = a_mc_p1_pm+n;
    const std::complex<long double> eps_pm_p1_pn = eps_pm_p1+n, n_p1_meps = one_meps+n;
    const std::complex<long double> eps_pa_pm_pn = eps_pa_pm+n;
    const std::complex<long double> eps_pa_mc_p1_pm_pn = eps_pa_mc_p1_pm+n; /*eps_pm_pn = eps_pm+n;*/
    const std::complex<long double> prod1 = eps_pa_pm_pn*eps_pa_mc_p1_pm_pn;
    const std::complex<long double> prod2 = eps_pm_p1_pn*n_p1, prod3 = a_pm_pn*a_mc_p1_pm_pn;
...
std::complex<long double> hyp_PS_complex_plane_rest(const std::complex<long double> &a,const std::complex<long double> &b,
        const std::complex<long double> &c,const std::complex<long double> &z)
{
    const long double abs_z = std::abs(z);
    const bool is_abs_z_small = abs_z<1.0;

    const std::complex<long double> z0 = (is_abs_z_small)?(0.9*z/abs_z):(1.1*z/abs_z), zc = z - z0;
    const std::complex<long double> zc_z0_ratio = zc/(z0*(1.0-z0)), z0_term1 = 2.0*z0-1.0;
    const std::complex<long double> z0_term2 = c-(a+b+1.0)*z0;

    const std::complex<long double> hyp_PS_z0 = (is_abs_z_small)?(hyp_PS_zero(a,b,c,z0)):(hyp_PS_infinity(a,b,c,z0));
    const std::complex<long double> dhyp_PS_z0 = (is_abs_z_small)?(hyp_PS_zero(a+1.0,b+1.0,c+1.0,z0)*a*b/c):
    ...
    {
      const std::complex<long double> anp2 = zc_z0_ratio*(anp1*(n*z0_term1-z0_term2)+an*zc*(a+n)*(b+n)/(n+1))/(n+2);
      sum += anp2;
      n++;
      an = anp1;
      anp1 = anp2;
...
std::complex<long double> Hyp2F1(std::complex<long double> a,std::complex<long double> b,
        std::complex<long double> c,std::complex<long double> z)
{
    const long double Re_a = std::real(a), Re_b = std::real(b), Re_c = std::real(c);
    ...
    const long double abs_z = abs(z), abs_z_inv = 1.0/abs_z, abs_z_over_zm1 = abs_z/abs_zm1;
    const long double abs_zm1_inv = 1.0/abs_zm1, abs_zm1_over_z = 1.0/abs_z_over_zm1;

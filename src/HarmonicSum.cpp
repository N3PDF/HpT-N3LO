...
std::complex<double> HSum::HS(int i, std::complex<double> N){
  switch (i){
    case(-4):
      return H_m4(N);
    case(-3):
...
std::complex<double> HSum::HS(int i,int j, std::complex<double> N){
  switch (i){
    case(-3):{
      switch (j){
	case(-1):
...
std::complex<double> HSum::HS(int i, int j, int k, std::complex<double> N){
  switch (i){
    case(-2):{
      switch(j){
	case(-1):{
...
std::complex<double> HSum::HS(int i,int j, int k, int m, std::complex<double> N){
  switch (i){
    case(-1):{
      switch (j){
	case(-1):{
...
std::complex<double> HSum::HS_int(int i,int n){
  double ris=0.;
  double neg=1.;
  if (i < 0){
    i=std::abs(i);
  ...
  std::complex<double> RIS(ris,0.);
  return RIS;
}

std::complex<double> HSum::HS_int(int i,int j,int n){
  double ris=0.,ris2=0.;
  double neg1=1.,neg2=1.;
  if (i < 0){
    i=std::abs(i);
    neg1=-1.;
  }
...
std::complex<double> HSum::HS_int(int i,int j,int k,int n){
  double ris=0.,ris2=0.,ris3=0.;
...
std::complex<double> HSum::HS_int(int i,int j,int k,int m,int n){
  double ris=0.,ris2=0.,ris3=0., ris4=0.;
  double neg1=1.,neg2=1.,neg3=1., neg4=1.;
  if (i < 0){
    i=std::abs(i);
    neg1=-1.;
  }
...
void HSum::TestInterpolatedFunction(){
  ...
  std::vector<std::complex<double>> g_v;
  std::complex<double> NTest=2.+II;
  std::complex<double> g_v_data[]={{0.0959473930360097,-0.028037163902461058},
...
void HSum::TestHarmonicSums(int n){
  ...
  std::vector<int> error;
  std::complex<double> N((double) n,0.);
  for (int i=-4;i<=4;i++){
    if ((i!=0)&&(std::abs(i)<5)){
      std::complex<double> DIFF=HS(i,N)-HS_int(i,n);
      if ((std::abs(std::real(DIFF))<2e-8)&&(std::abs(std::imag(DIFF))<2e-8)){
      }
      else{
	test1=false;
  ...
  for (int i=-3;i<=3;i++){
	...
	std::complex<double> DIFF=HS(i,j,N)-HS_int(i,j,n);
	if ((std::abs(std::real(DIFF))<2e-8)&&(std::abs(std::imag(DIFF))<2e-8)){
	}
	else{
	  test2=false;
  ...
  for (int i=-2;i<=2;i++){
	...
	if ((i!=0)&&(j!=0)&&(k!=0)&&(std::abs(k)+std::abs(j)+std::abs(i)<5)){
	  std::complex<double> DIFF=HS(i,j,k,N)-HS_int(i,j,k,n);
	  if ((std::abs(std::real(DIFF))<2e-8)&&(std::abs(std::imag(DIFF))<2e-8)){
	  }
	  else{
	    test3=false;
  ...
  for (int i=-1;i<=1;i++){
	...
	for(int m=-1;m<=1;m++){
	  if ((i!=0)&&(j!=0)&&(k!=0)&&(m!=0)&&(std::abs(i)+std::abs(j)+std::abs(k)+std::abs(m)<5)){
	    std::complex<double> DIFF=HS(i,j,k,m,N)-HS_int(i,j,k,m,n);
	    if ((std::abs(std::real(DIFF))<2e-8)&&(std::abs(std::imag(DIFF))<2e-8)){
	      }
	    else{
	      test4=false;
...
void HSum::InizializeConst(){
  ...
  double a1_data[]={0.999999974532238,-0.499995525889840,0.333203435557262,
  ...
  double a2_data[]={0.,0.999999980543793,-0.999995797779624,0.916516447393493,
  ...
  double a3_data[]={0.,0.,0.999999989322696,-1.49999722020708,1.74988008499745,
  ...
  double b1_data[]={0.693147166991375,-0.306850436868254,0.193078041088284,
  ...
  double b2_data[]={0.480453024731510,0.480450679641120,-0.519463586324817,
  ...
  double b3_data[]={0.33302465198526926,0.33302465294458333,0.33302458698245874,
  ...
  double c1_data[]={2.2012182965269744e-8,2.833327652357064,-1.8330909624101532,
  ...
  double P21_data[]={11./6.,-3.,3./2.,-1./3.};
   P21.insert(P21.end(),&P21_data[0],&P21_data[sizeof(P21_data)/(sizeof(P21_data[0]))]);

  double c2_data[]={1.989197140339627e-8,-6.050453690953361e-6,2.12530461606213,
  ...
  double P22_data[]={-1.,5./2.,-2.,1./2.};
  P22.insert(P22.end(),&P22_data[0],&P22_data[sizeof(P22_data)/(sizeof(P22_data[0]))]);

  double c3_data[]={-2.878388705163104e-8,1.423616247405256,-0.08001203559240111,
  ...
  double P23_data[]={205./144.,-25./12.,23./24.,-13./36.,1./16.};
  P23.insert(P23.end(),&P23_data[0],&P23_data[sizeof(P23_data)/(sizeof(P23_data[0]))]);
  double P33_data[]={-25./24.,2.,-3./2.,2./3.,-1./8.};
  P33.insert(P33.end(),&P33_data[0],&P33_data[sizeof(P33_data)/(sizeof(P33_data[0]))]);

  double c4_data[]={-1.84461401708802e-8,2.2150086978693073,-0.9133677154535804,
  ...
  double P24_data[]={-167./36.+25./6.*zeta2,235./18.-8.*zeta2,-40./3.+6.*zeta2,109./18.-8./3.*zeta2,-41./36.+1./2.*zeta2};
  P24.insert(P24.end(),&P24_data[0],&P24_data[sizeof(P24_data)/(sizeof(P24_data[0]))]);
  double P34_data[]={35./12.,-26./3.,19./2.,-14./3.,11./12.};
  P34.insert(P34.end(),&P34_data[0],&P34_data[sizeof(P34_data)/(sizeof(P34_data[0]))]);

  double c5_data[]={-0.822467033400776,0.0887664705657325,-0.0241549406045162,
  ...
  double d5_data[]={-0.822467033400776,0.999999974532241,-0.249997762945014,
  ...
  double q1_data[]={-0.09475300423010691,0.1454735027234842,-0.08098683369213346,
  ...
  double q2_data[]={0.537213193604428,-1.119453718988143,1.174906969846667,
  ...
  double q3_data[]={-0.5372131936080373,0.045027332856608177,-0.010425994173158348,
  ...
  double q4_data[]={0.0947530042271157,-0.3349795102848428,0.5614397835582168,
  ...
  double q5_data[]={1.4092730215810628e-11,0.5822405222318227,-1.5665076685110302,
  ...
  double q6_data[]={-2.3184816518117378e-11,6.968818347250131e-9,0.9999995028321177,
  ...
  double q7_data[]={-0.5822405264650113,0.11090665409384426,-0.042519755533205965,
...
std::complex <double> HSum::g1(std::complex<double > N){
...
std::complex <double> HSum::g2(std::complex<double > N){
std::complex<double> sum;
...
std::complex <double> HSum::g3(std::complex<double > N){
std::complex<double> sum;
...
std::complex <double> HSum::g4(std::complex<double > N){
std::complex<double> sum;
for (int i=0;i<static_cast<int>(a1.size());i++){
  std::complex<double> k((double) i,0.);
...
std::complex <double> HSum::g5(std::complex<double > N){
std::complex<double> sum;
for (int i=0;i<static_cast<int>(a1.size());i++){
  std::complex<double> k((double) i,0.);
...
std::complex <double> HSum::g6(std::complex<double > N){
std::complex<double> sum;
for (int i=0;i<static_cast<int>(a1.size());i++){
  std::complex<double> k((double) i,0.);
...
std::complex <double> HSum::g7(std::complex<double > N){
std::complex<double> sum;
for (int i=0;i<static_cast<int>(a1.size());i++){
  std::complex<double> k((double) i,0.);
...
std::complex <double> HSum::g8(std::complex<double > N){
std::complex<double> sum;
for (int i=0;i<static_cast<int>(a1.size());i++){
  std::complex<double> k((double) i,0.);
...
std::complex <double> HSum::g9(std::complex<double > N){
std::complex<double> sum,sum3;
for (int i=0;i<static_cast<int>(a1.size());i++){
...
std::complex <double> HSum::g10(std::complex<double > N){
std::complex<double> sum,sum3;
for (int i=0;i<static_cast<int>(a1.size());i++){
  std::complex<double> sum2;
  std::complex<double> k((double) i,0.);
...
for (int h=0;h<static_cast<int>(a2.size());h++){
  std::complex<double> m((double) h,0.);
  sum3+=a2[h]/(N+m+1.)*H_1(N+m+1.);
}
return (-5./8.*zeta3*log2+sum+sum3);
}

std::complex <double> HSum::g11(std::complex<double > N){
...
std::complex <double> HSum::g12(std::complex<double > N){
  std::complex<double> sum;
  for (int i=0;i<static_cast<int>(a1.size());i++){
    std::complex<double> k((double)i,0.);
    std::complex<double> sum2;
...
std::complex <double> HSum::g13(std::complex<double > N){
  std::complex<double> sum;
  for (int i=0;i<static_cast<int>(a3.size());i++){
    std::complex<double> k((double) i,0.);
    sum+=a3[i]/(N+k+1.);
...
std::complex <double> HSum::g14(std::complex<double > N){
  std::complex<double> sum;
...
std::complex <double> HSum::g15(std::complex<double > N){
  std::complex<double> sum;
...
std::complex <double> HSum::g17(std::complex<double > N){
  std::complex<double> sum;
  for (int i=0;i<static_cast<int>(b2.size());i++){
    std::complex<double> k((double) i,0.);
...
//Weight 1
std::complex<double> HSum::H_1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_m1(std::complex<double> N){
...
std::complex<double> HSum::H_1_1(std::complex<double> N){
  return(0.5*(H_1(N)*H_1(N)+H_2(N)));
}

//Weight 3

std::complex<double> HSum::H_3(std::complex<double> N){
...
std::complex<double> HSum::H_m2_m1(std::complex<double> N){
...
std::complex<double> HSum::H_2_1(std::complex<double> N){
  return(-g18(N)+2.*zeta3);
}

std::complex<double> HSum::H_m1_m2(std::complex<double> N){
  return(-H_m2_m1(N)+H_m2(N)*H_m1(N)+H_3(N));
}

std::complex<double> HSum::H_m1_2(std::complex<double> N){
  return(-H_2_m1(N)+H_2(N)*H_m1(N)+H_m3(N));
}

std::complex<double> HSum::H_1_2(std::complex<double> N){
  return(-H_2_1(N)+H_2(N)*H_1(N)+H_3(N));
}

std::complex<double> HSum::H_1_m2(std::complex<double> N){
  return(-H_m2_1(N)+H_m2(N)*H_1(N)+H_m3(N));
}

std::complex<double> HSum::H_m1_m1_m1(std::complex<double> N){
  return(1./6.*(2.*H_m3(N)+std::pow(H_m1(N),3.)+3.*H_m1(N)*H_2(N)));
}

std::complex<double> HSum::H_m1_1_m1(std::complex<double> N){
...
std::complex<double> HSum::H_m2_m2(std::complex<double> N){
...
std::complex<double> HSum::H_2_m2(std::complex<double> N){
  return(-H_m2_2(N)+H_m4(N)+H_m2(N)*H_2(N));
}

std::complex<double> HSum::H_2_2(std::complex<double> N){
...
std::complex<double> HSum::H_m3_m1(std::complex<double> N){
  return(g23(N)+log2*(H_3(N)-H_m3(N))-0.5*zeta2*H_2(N)-2.*Li4+11./10.*zeta2q-7./4.*zeta3*log2+0.5*zeta2*log2q-1./12.*log2c*log2);
}

std::complex<double> HSum::H_3_1(std::complex<double> N){
  return(0.5*g22(N)-1./4.*H_4(N)-1./4.*H_2(N)*H_2(N)+zeta2*H_2(N)-3./20.*zeta2q);
}

std::complex<double> HSum::H_m1_m3(std::complex<double> N){
  return(-H_m3_m1(N)+H_m3(N)*H_m1(N)+H_4(N));
}

std::complex<double> HSum::H_m1_3(std::complex<double> N){
  return(-H_3_m1(N)+H_3(N)*H_m1(N)+H_m4(N));
}

std::complex<double> HSum::H_1_m3(std::complex<double> N){
  return(-H_m3_1(N)+H_m3(N)*H_1(N)+H_m4(N));
}

std::complex<double> HSum::H_1_3(std::complex<double> N){
...
std::complex<double> HSum::H_m2_1_m1(std::complex<double> N){
  return(-g25(N)-log2*(H_m2_1(N)-H_m2_m1(N))+0.5*log2q*(H_m2(N)-H_2(N))+3./40.*zeta2q);
}

std::complex<double> HSum::H_m2_m1_m1(std::complex<double> N){
  return(0.5*(-H_m1_m2_m1(N)+H_m2_2(N)+H_3_m1(N)+H_m1(N)*H_m2_m1(N)));
}


std::complex<double> HSum::H_m2_m1_1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_2_m1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_1_m2(std::complex<double> N){
...
std::complex<double> HSum::H_2_m1_m1(std::complex<double> N){
  return(0.5*(-H_m1_2_m1(N)+H_m3_m1(N)+H_m1(N)*H_2_m1(N)+H_2_2(N)));
}

std::complex<double> HSum::H_m1_m1_2(std::complex<double> N){
  return(0.5*(-H_m1_2_m1(N)+H_m1_m3(N)+H_m1(N)*H_m1_2(N)+H_2_2(N)));
}

std::complex<double> HSum::H_2_1_1(std::complex<double> N){
  return(-g21(N)+6./5.*zeta2q);
}

std::complex<double> HSum::H_1_2_1(std::complex<double> N){
  return(-2.*H_2_1_1(N)+H_3_1(N)+H_1(N)*H_2_1(N)+H_2_2(N));
}

std::complex<double> HSum::H_1_1_2(std::complex<double> N){
  return(H_2_1_1(N)+0.5*(H_1(N)*(H_1_2(N)-H_2_1(N))+H_1_3(N)-H_3_1(N)));
}

std::complex<double> HSum::H_1_m2_1(std::complex<double> N){
  return(-2.*H_m2_1_1(N)+H_m3_1(N)+H_1(N)*H_m2_1(N)+H_m2_2(N));
}

std::complex<double> HSum::H_1_1_m2(std::complex<double> N){
...
std::complex<double> HSum::H_m1_m2_1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_m1_m1_m1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_1_1_m1(std::complex<double> N){
...
std::complex<double> HSum::H_1_m1_1_m1(std::complex<double> N){
...
std::complex<double> HSum::H_1_1_m1_m1(std::complex<double> N){
...
std::complex<double> HSum::H_1_1_1_1(std::complex<double> N){
...
std::complex<double> HSum::H_1_m1_1_1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_m1_m1_1(std::complex<double> N){
...
std::complex<double> HSum::H_1_m1_m1_1(std::complex<double> N){
  return(g38(N)-(2.*H_1_1_m1_m1(N)+1./12.*H_1_m1(N)*6.*(zeta2-log2q)+H_1_1(N)*(-zeta2+log2q)+H_1_1_m1(N)*log2*2.+Li4) );
}

std::complex<double> HSum::H_m1_m1_1_1(std::complex<double> N){
...
std::complex<double> HSum::H_m1_1_m1_1(std::complex<double> N){

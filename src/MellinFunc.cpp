...
std::complex<double> MellinFunc::Li3zplus(std::complex<double> N){
  double frac,n=0.;
...
std::complex<double> MellinFunc::S12z2plus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::S12mzplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::S12zplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(pow(-1.,n)*(H.HS(-2,1,1,N-1.)-zeta3*H.HS(-1,N-1.)+Li4-1./8.*zeta2q-1./8.*zeta3*log2-1./4.*zeta2*log2q+1./24.*log2q*log2q));
}
std::complex<double> MellinFunc::Li3mzplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li2zLogminus(std::complex<double> N){
  return(1./N*(-2.*zeta3-zeta2*H.HS(1,N)+1./N*(H.HS(1,N)*H.HS(1,N)+H.HS(2,N))+H.HS(2,1,N)));
}

std::complex<double> MellinFunc::Li2zLogplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li2mzLogplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li2mzLogz(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(1./(N*N)*0.5*zeta2-2./(N*N*N)*log2+pow(-1.,n)*(1./(N*N)*(0.5*zeta2+H.HS(-2,N))+1./(N*N*N)*(2.*H.HS(-1,N)+2.*log2)));
}
std::complex<double> MellinFunc::Li2zLogzplusminus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li2mzLogzplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::LogzLogminus2minus(std::complex<double> N){
...
std::complex<double> MellinFunc::Li2mzLogplusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Logz2Logminusminus(std::complex<double> N){
  return(-2.*zeta3*H.HS(1,N-1.)+2.*H.HS(1,N-1.)*H.HS(3,N-1.)-2.*zeta2*H.HS(2,N-1.)+H.HS(2,N-1.)*H.HS(2,N-1.)+3.*H.HS(4,N-1.)-zeta2q/5.);
}
std::complex<double> MellinFunc::Logz3minusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(pow(-1.,n)*(21./20.*zeta2q+3.*H.HS(-4,N-1.))-6./5.*zeta2q+3.*H.HS(4,N-1.));
}
std::complex<double> MellinFunc::Logplusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(pow(-1.,n)*(H.HS(1,-1,N-1.)-0.5*log2q+(-H.HS(-1,N-1.)+H.HS(1,N-1.))*log2));
}
std::complex<double> MellinFunc::Li2zLogplusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Logz2Logplusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::LogzLogplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(1./12./(N*N)*(-12.*log2+pow(-1.,n)*(6.*N*zeta2+12.*N*H.HS(-2,N)+12.*H.HS(-1,N)+12.*log2)));
}
std::complex<double> MellinFunc::Logz2Logplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(1./(N*N*N)*2.*log2-pow(-1.,n)*(1./(N*N)*(zeta2+2.*H.HS(-2,N))+1./(N*N*N)*(2.*H.HS(-1,N)+2.*log2)+1./N*(2.*H.HS(-3,N)+3./2.*zeta3)));
}
std::complex<double> MellinFunc::LogzLogplus2(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::LogzLogplus2plus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li2z(std::complex<double> N){
  return(1./N*zeta2-1./(N*N)*H.HS(1,N));
}
std::complex<double> MellinFunc::Li2mz(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(-0.5*zeta2/N-1./(N*N)*(pow(-1.,n)*(H.HS(-1,N)+log2)-log2));
}
std::complex<double> MellinFunc::S12z(std::complex<double> N){
  return(-1./(2.*N*N)*(H.HS(1,N)*H.HS(1,N)+H.HS(2,N)-2.*N*zeta3));
}
std::complex<double> MellinFunc::S12mz(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(1./(8.*N*N)*(-8.*pow(-1.,n)*H.HS(1,-1,N)+8.*pow(-1.,n)*H.HS(-1,N)*log2
  -8.*pow(-1.,n)*H.HS(1,N)*log2-4.*log2q+4.*pow(-1.,n)*log2q+N*zeta3));
}
std::complex<double> MellinFunc::S12z2(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li3z(std::complex<double> N){
  return(1./(N*N*N)*H.HS(1,N)-1./(N*N)*zeta2+1./N*zeta3);
}
std::complex<double> MellinFunc::Li3mz(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(pow(-1.,n)*1./(N*N*N)*(log2+H.HS(-1,N))-log2/(N*N*N)+0.5/(N*N)*zeta2-3./4./N*zeta3);
}
std::complex<double> MellinFunc::Li2zLogz(std::complex<double> N){
  return(2./(N*N*N)*H.HS(1,N)+1./(N*N)*H.HS(2,N)-2./(N*N)*zeta2);
}
std::complex<double> MellinFunc::Logminus(std::complex<double> N){
  return(-H.HS(1,N)/N);
}
std::complex<double> MellinFunc::Logplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(1./N*(log2-pow(-1.,n)*(H.HS(-1,N)+log2)));
}
std::complex<double> MellinFunc::Logz(std::complex<double> N){
  return(-1./(N*N));
}
std::complex<double> MellinFunc::Logz2(std::complex<double> N){
  return(2./(N*N*N));
}
std::complex<double> MellinFunc::Logz3(std::complex<double> N){
  return(-6./(N*N*N*N));
}
std::complex<double> MellinFunc::LogzLogminus(std::complex<double> N){
  return(-1./N*zeta2+1./N*H.HS(2,N)+1./(N*N)*H.HS(1,N));
}
std::complex<double> MellinFunc::LogzLogminus2(std::complex<double> N){
  return(1./(N*N)*(-H.HS(1,N)*H.HS(1,N)+2.*N*H.HS(1,N)*(zeta2-H.HS(2,N))-H.HS(2,N)-2.*N*H.HS(3,N)+2.*N*zeta3));
}
std::complex<double> MellinFunc::Logz2Logminus(std::complex<double> N){
  return(-2./(N*N*N)*H.HS(1,N)+1./(N*N)*(2.*zeta2-2.*H.HS(2,N))-2./N*(H.HS(3,N)-zeta3));
}
std::complex<double> MellinFunc::Logz2minus(std::complex<double> N){
  return(2.*(-H.HS(3,N-1.)+zeta3));
}
std::complex<double> MellinFunc::Logminus2(std::complex<double> N){
  return(1/N*(H.HS(1,N)*H.HS(1,N)+H.HS(2,N)));
}
std::complex<double> MellinFunc::Logminus3(std::complex<double> N){
  return(-1./N*(H.HS(1,N)*H.HS(1,N)*H.HS(1,N)+3.*H.HS(1,N)*H.HS(2,N)+2.*H.HS(3,N)));
}
std::complex<double> MellinFunc::LogzLogminusminus(std::complex<double> N){
  return(zeta3+zeta2*H.HS(1,N-1.)-H.HS(1,N-1.)*H.HS(2,N-1.)-H.HS(3,N-1.));
}
std::complex<double> MellinFunc::Logzminus(std::complex<double> N){
  return(-zeta2+H.HS(2,N-1.));
}
std::complex<double> MellinFunc::Logzminusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(pow(-1.,n)*(0.25*zeta2+0.5*H.HS(-2,N-1.))+0.5*H.HS(2,N-1.)-0.5*zeta2);
}
std::complex<double> MellinFunc::plus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(-pow(-1.,n)*(H.HS(-1,N-1.)+log2));
}
std::complex<double> MellinFunc::Li2minusminus(std::complex<double> N){
  return(-zeta2*H.HS(1,N-1.)+H.HS(1,2,N-1.)+zeta3);
}
std::complex<double> MellinFunc::S12zregminus(std::complex<double> N){
  return(-6./5.*zeta2q+H.HS(2,1,1,N-1.));
}
std::complex<double> MellinFunc::Li2zregminus(std::complex<double> N){
  return(H.HS(2,1,N-1.)-2.*zeta3);
}
std::complex<double> MellinFunc::Li3zregminus(std::complex<double> N){
  return(-0.5*zeta2q+zeta2*H.HS(2,N-1.)-H.HS(3,1,N-1.));
}
std::complex<double> MellinFunc::Li2zregLogminusminus(std::complex<double> N){
  return(6./5.*zeta2q-H.HS(1,2,1,N-1.)-2.*H.HS(2,1,1,N-1.)+2.*H.HS(1,N-1.)*zeta3);
}

std::complex<double> MellinFunc::Logplus3plus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li3zregminusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return (1./720.*(-5.*(pow(M_PI,4)-12.*pow(M_PI,2)*H.HS(2,N-1.)+72.*H.HS(3,1,N-1.))-6.*pow(-1.,n)*(pow(M_PI,4)
  -10.*pow(M_PI,2)*H.HS(-2,N-1.)+60.*H.HS(-3,1,N-1.)+5.*M_PI*M_PI*log2q-5.*(log2q*log2q+24.*Li4+21.*log2*zeta3))));
}
std::complex<double> MellinFunc::Li3zoverplusplus(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return (1./288.*pow(-1.,n)*(pow(M_PI,4)+288.*H.HS(3,-1,N-1.)-288.*H.HS(1,2,-1,N-1.)-288.*H.HS(2,1,-1,N-1.)+288.*H.HS(1,1,1,-1,N-1.)
  +24.*H.HS(-2,N-1.)*(M_PI*M_PI-6.*log2q)-24.*H.HS(1,-1,N-1.)*(M_PI*M_PI-6.*log2q)-12.*log2*(24.*H.HS(-3,N-1.)
  -4.*pow(H.HS(1,N-1.),3)-8.*(H.HS(3,N-1.)+3.*(H.HS(1,-2,N-1.)+H.HS(2,-1,N-1.)-H.HS(1,1,-1,N-1.)))
  -(M_PI*M_PI+6.*H.HS(2,N-1.))*log2+log2c-2.*H.HS(-1,N-1.)*(M_PI*M_PI-2.*log2q)
  +2.*H.HS(1,N-1.)*(M_PI*M_PI+6.*H.HS(2,N-1.)-2.*log2q)+H.HS(1,N-1.)*H.HS(1,N-1.)*6.*log2)
  -36.*(7.*H.HS(-1,N-1.)-2.*H.HS(1,N-1.)+7.*log2)*zeta3));
}
std::complex<double> MellinFunc::Logplus3(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return (log2c-pow(-1.,n)*(6.*H.HS(1,1,-1,N)+log2*(3.*(H.HS(1,N)*H.HS(1,N)+H.HS(2,N))
  -6.*H.HS(1,-1,N)+log2q+(H.HS(-1,N)-H.HS(1,N))*3.*log2)))/N;
}
std::complex<double> MellinFunc::Li2z2(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return (N*M_PI*M_PI-12.*H.HS(1,N)-12.*pow(-1.,n)*(H.HS(-1,N)+log2)+12.*log2)/(6.*N*N);
}
std::complex<double> MellinFunc::Li2z2Logz(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return(pow(-1.,n)*(N*M_PI*M_PI+12.*N*H.HS(-2,N)+24.*H.HS(-1,N)+24.*log2)-3.*(N*M_PI*M_PI-8.*H.HS(1,N)
  -4.*N*H.HS(2,N)+8.*log2))/(6.*N*N*N);
}
std::complex<double> MellinFunc::Li3z2(std::complex<double> N){
  double frac,n=0.;
  frac=modf(std::real(N),&n);
  return (-N*M_PI*M_PI+12.*H.HS(1,N)-12.*log2+12.*pow(-1.,n)*(H.HS(-1,N)+log2)+3.*N*N*zeta3)/(3.*N*N*N);
}
std::complex< double > MellinFunc::Li3overplus(std::complex< double > N) {
  double frac,n=0.;
  frac=modf(std::real(N),&n);
...
std::complex<double> MellinFunc::Li2minus(std::complex<double> N){
  return( zeta2/N-H.HS(2,N)/N);
}


std::complex<double> MellinFunc::D0(std::complex<double> N){
  return(-H.HS(1,N-1.));
}
std::complex<double> MellinFunc::D1(std::complex<double> N){
  return(H.HS(1,1,N-1.));
}
std::complex<double> MellinFunc::D2(std::complex<double> N){
  return(-2.*H.HS(1,1,1,N-1.));
}
std::complex<double> MellinFunc::D3(std::complex<double> N){

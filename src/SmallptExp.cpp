...
long double SmallptExp::LC1(long double xp) { return -1. / xp; }
...
SmallptExp::SmallptExpExpr(std::complex<long double> N, long double pt) {
  ...
  case (0): // order as^1
  {
    if ((CHANNEL == 0) || (CHANNEL == 4)) // gg-channel or ALL
    {
      ...
      std::complex<long double> HH1GG =
          h1gg + 2. * C1GG(N) + 2. * (LF - LQ) * AD.gg0 -
  ...
  case (1): // order as^2
  {
    if ((CHANNEL == 0) || (CHANNEL == 4)) // gg-channel or ALL
    {
      ...
      std::complex<long double> Sigma23gg =
          -Apt1g * (Beta0 / 3. + 1. / 2. * (-Bpt1g - Apt1g * LQ - 2. * AD.gg0));
      std::complex<long double> Sigma22gg =
          (-Apt2g -
           (-Bpt1g - 2. * AD.gg0 - Apt1g * LQ) * (Bpt1g - Beta0 + Apt1g * LQ)) /
              2. -
          (Apt1g * (2 * C1GG(N) + h1gg + 2. * AD.gg0 * (LF - LQ) -
                    LQ * (Bpt1g + (Bpt1g * LQ) / 2.) - (Beta0 * (-LQ + LR)) -
                    Beta0 * LR * pc)) /
              2. +
          (-2. * AD.gg0 * (-Bpt1g - 2. * AD.gg0 - Apt1g * LQ) +
           2. * AD.gq0 * AD.qg0) /
              2.;
      std::complex<long double> Sigma21gg =
          -Bpt2g + 2. * Beta0 * C1GG(N) - 2. * AD.gg1 - Apt2g * LQ +
          Beta0 * (-Bpt1g - AD.gg0 - Apt1g * LQ) * (LQ - LR) -
          (Bpt1g + 2. * AD.gg0 + Apt1g * LQ) *
              (2. * C1GG(N) + h1gg + 2. * AD.gg0 * (LF - LQ) -
               LQ * (Bpt1g + (Bpt1g * LQ) / 2.) - Beta0 * LR * pc) -
          2. * (C1GQ(N) + AD.gq0 * (LF - LQ)) * AD.qg0;
      ...
      // NLO contributions
      result += aass * aass *
                (Sigma24gg * LC4(xp) + Sigma23gg * LC3(xp) +
                 Sigma22gg * LC2(xp) + Sigma21gg * LC1(xp) + HH2GG);
    }
    if ((CHANNEL == 1) || (CHANNEL == 4)) {
      result += zero;
    }
